...
class WFLinkBrowser_SobiProlinks extends JObject
{
	...
	*/
	function __construct($options = array()){
		...
		$path = dirname( __FILE__ ) .DS. 'sobiprolinks';
	...
	function &getInstance(){
		...
		if ( !is_object( $instance ) ){
			$instance = new WFLinkBrowser_Sobiprolinks();
		}
		return $instance;
	}
	...
	function display()
	{
		...
		$document->addStyleSheet(array('sobiprolinks'), 'extensions/links/sobiprolinks/css');
	}
	
	function isEnabled() 
	{
		$wf = WFEditorPlugin::getInstance();
		return $wf->checkAccess($wf->getName() . '.links.sobiprolinks', 1);
	}
